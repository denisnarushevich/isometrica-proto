	...
	objects: {
		...
		fill: function(){
			...
			for(var key in tiles){
				var item = tiles[key];
				var item = tile.getObject();
				if(item){
					item.sceneOffset = tile.sceneOffset.slice();
					item.sceneOffset[1] = item.sceneOffset[1] - item.sprite.image.height  + g.config.tileSpriteSize[1] - g.config.tileSpriteSize[2];
...
//		var key = 0;
//		while(item = this.objects.updatedItems[key++]){
//			g.render.drawSprite('objects', [item.sprite], item.sceneOffset[0], item.sceneOffset[1]);
//		}
